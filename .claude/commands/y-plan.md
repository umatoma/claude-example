...
このコマンドは **y-planner** エージェントを呼び出し、コードを書く前に包括的な実装計画を作成します。
...
```
ユーザー: /y-plan マーケットが解決したときにリアルタイム通知を追加したい
...
**重要**: y-plannerエージェントは、「はい」「進めてください」などの明示的な確認を受けるまで、**実装は進めません**。ただし、計画書の作成は行います。
...
計画後：
- 計画書に基づいて実装するには `/y-implement` を使用

## 関連エージェント

このコマンドは以下の場所にある `y-planner` エージェントを呼び出します：
`.claude/agents/y-planner.md`
