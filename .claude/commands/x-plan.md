---
description: SDD実装計画。仕様書（Part 1）を読み込み、実装計画（Part 2）を追記します。ユーザー承認後に /x-implement へ進みます。
---
...
**未通過の場合**: 未達成の条件を報告し、`/x-specify` に戻ることを提案して中止する。
...
承認後の次のステップ: `/x-implement` で実装を開始する。
...
- 前のステップ: `/x-specify` - 仕様書の作成
- 次のステップ: `/x-implement` - 計画に基づいて実装
